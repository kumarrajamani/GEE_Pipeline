...
class LSTM(torch.nn.Module):
    def __init__(self, input_dim=13, num_classes=9, hidden_dims=128, num_layers=4, dropout=0.5713020228087161, bidirectional=True, use_layernorm=True):
        ...
        if bidirectional:
         ...
         # Define fully connected layers before the linear class
        self.fc1 = nn.Linear(hidden_dims * num_layers, hidden_dims)  # First fully connected layer
        self.fc2 = nn.Linear(hidden_dims, hidden_dims)                # Second fully connected layer
    ...
    def logits(self, x):
        ...
        h = self.fc1(h)  # Applying the first fully connected layer
        h = F.relu(h)    # Applying ReLU activation
        h = self.fc2(h)  # Applying the second fully connected layer
        h = F.relu(h)    # Applying ReLU activation
